...
def interpret_args(args, dash_dash, default_commit):
  ...
  if dash_dash:
    commits = [default_commit] if len(args) == 0 else args
...
def get_object_type(value):
  ...
  return None if p.returncode != 0 else convert_string(stdout.strip())
...
def compute_diff(commits, files):
  ...
  git_tool = 'diff-tree' if len(commits) > 1 else 'diff-index'
...
def clang_format_to_blob(filename, line_ranges, revision=None,
  ...
  if revision:
    clang_format_cmd.extend([f'-assume-filename={filename}'])
    git_show_cmd = ['git', 'cat-file', 'blob', f'{revision}:{filename}']
  ...
  if git_show and git_show.wait() != 0:
    die(f"`{' '.join(git_show_cmd)}` failed")
...
def apply_changes(old_tree, new_tree, force=False, patch_mode=False):
  ...
  if not force:
    if unstaged_files := run('git', 'diff-files', '--name-status',
                             *changed_files):
...
def to_bytes(str_input):
  ...
  return str_input if isinstance(str_input, bytes) else str_input.encode('utf-8')
